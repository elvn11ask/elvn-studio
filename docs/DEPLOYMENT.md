...
2. Create `shared/studio.env` from the documented variable names and `shared/runtime.env` containing the selected loopback port. Keep both outside the repository.
3. Install `deploy/elvn-studio-deploy` as `/usr/local/sbin/elvn-studio-deploy`, owned by root and not writable by the deploy account.
4. Permit the restricted deploy account to run only that script with the two required arguments.
5. Install the Nginx server block after replacing `__STUDIO_PORT__`, validate with `nginx -t`, and create a rollback copy before reloading.
6. Confirm DNS, issue the certificate once, and validate renewal.
7. Create the GitHub `production` Environment and add only the documented deployment secrets.
