...
export default function ValidationPage() {
  return <>
    ...
    <section className="section shell"><div className="section-heading"><div><p className="eyebrow">Methodology · measured {catalogGuardValidation.measuredAt}, {catalogGuardValidation.measuredAtUtc}</p><h2>A bounded engineering validation—not a customer benchmark.</h2></div><p>The cohorts were selected from five authorized first-party public environments. Requests ran sequentially against approved public URLs only.</p></div><div className="ros-module-grid"><article><span>MEASURED FACT</span><h3>95 successful page requests</h3><p>75 product pages and 20 general commercial pages. The run recorded zero request errors, form submissions, authenticated requests or production writes.</p></article><article><span>LIMITATION</span><h3>Server-observable scope</h3><p>HTTP, redirects, indexability, canonical, metadata, headings, schema types, important links and network timing. No conversion, ranking, Core Web Vitals or revenue measurement.</p></article><article><span>FUTURE CAPABILITY</span><h3>Rendered browser cohort</h3><p>Deep rendered-DOM, mobile and visual comparison will be published only after a separately bounded adapter cohort passes the same evidence rules.</p></article></div></section>
