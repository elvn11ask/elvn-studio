...
export default function sitemap():MetadataRoute.Sitemap{return [...staticRoutes.map((route)=>({url:`${site.url}${route}`,lastModified:route.includes("manufacturer-intelligence")?MANUFACTURER_INTELLIGENCE_UPDATED:route.includes("knowledge-graph")?GRAPH_UPDATED:BASE_UPDATED,changeFrequency:route===""?"weekly" as const:"monthly" as const,priority:route===""?1:route==="/work"?0.9:route==="/revenueos/knowledge-graph"||route==="/revenueos/manufacturer-intelligence"?0.9:0.7})),...getProjects().map((project)=>({url:`${site.url}/work/${project.slug}`,lastModified:BASE_UPDATED,changeFrequency:"monthly" as const,priority:0.8}))]}
